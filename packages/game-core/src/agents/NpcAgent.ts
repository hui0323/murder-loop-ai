...
export const NpcAgent: AgentRegistration = {
  id: 'npc',
  subscriptions: [{ event: 'ActionParsed', priority: 25 }],
  contract: {
    version: '1.0.0',
    input: null as never,
  ...
  fallback: async (input: unknown) => {
    const payload = input as { plan?: { intents?: Array<{ intent: string; target?: string; detail?: string }> }; state: GameState };
    const plan = payload.plan;
    const state = payload.state;
    // 检查是否有 NPC 通信意图
    const commIntent = plan?.intents?.find((i) => i.intent === 'communicate');
    if (commIntent?.target) {
      const speaker = commIntent.target as NpcReply['speaker'];
      return fallbackNpcReply(speaker, commIntent.detail ?? '', state);
    }
    return null;
  },
  mode: 'fallback',
